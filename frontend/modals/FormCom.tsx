...
type Props = ModalProps & {
  onSuccess?: () => void; // fonction optionnelle à appeler après ajout réussi
};

export default function FormCom({ onClose, onSuccess }: Props) {
      // États
    ...
    const ajouterCom = async () => {
        ...
        if (username === "" || email === "" || mdp === "" || confmdp === "") {
            ...
            const result = await ajouterCommunication(username, email, mdp);
            if (result.success) {
                alert("Ajout de nouvelle communication réussit")
                onClose?.(); // Ferme le modal si l'ajout est réussi
                onSuccess?.(); //Mise à jour du tableau de liste communication
            } else {
    ...
    return (
        ...
        <MDBCard className="shadow rounded">
            ...
            <div className="d-flex justify-content-end mt-3">
                ...
                <MDBBtn onClick={ajouterCom}>Ajouter</MDBBtn>
            </div>
            </MDBCardBody>
        </MDBCard>
        </div>
